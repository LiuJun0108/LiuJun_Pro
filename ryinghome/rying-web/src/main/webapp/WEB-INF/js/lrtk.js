	...
	$('.og_next2').click(function(){
		...
		if($('.mainlist2 li').length>4){//多于4张图片
			...
			if(ml2<=0 && ml2>w2*-1){//默认图片显示时
				...
				$('.mainlist2').animate({left: ml2 - 1000 + 'px'},'slow');//默认图片滚动				
				if(ml2==(w2-1000)*-1){//默认图片最后一屏时
					$('.swaplist2').animate({left: '0px'},'slow');//交换图片滚动
				}
			}else{//交换图片显示时
				$('.mainlist2').css({left: '1000px'})//默认图片放在显示区域右
				$('.swaplist2').animate({left: sl2 - 1000 + 'px'},'slow');//交换图片滚动
				if(sl2==(w2-1000)*-1){//交换图片最后一屏时
	...
	$('.og_prev2').click(function(){
		...
		if($('.mainlist2 li').length>4){
			...
			if(ml2<=0 && ml2>w2*-1){
				...
				$('.mainlist2').animate({left: ml2 + 1000 + 'px'},'slow');				
				if(ml2==0){
					$('.swaplist2').animate({left: (w2 - 1000) * -1 + 'px'},'slow');
				}
			}else{
				$('.mainlist2').css({left: (w2 - 1000) * -1 + 'px'});
				$('.swaplist2').animate({left: sl2 + 1000 + 'px'},'slow');
				if(sl2==0){
